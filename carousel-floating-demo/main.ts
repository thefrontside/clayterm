...
function frameTravel(metrics: LayoutMetrics): number {
  return metrics.frameOuterWidth + Math.max(4, Math.floor(metrics.frameOuterWidth * 0.14));
}

function frameBaseY(metrics: LayoutMetrics): number {
...
function buildOps(): Op[] {
  ...
  let displayIndex = state.transition?.toIndex ?? state.currentSlide;
  let indicator = `${padLeft(String(displayIndex + 1), 2)} / ${slideSpecs.length}`;
  let ops: Op[] = [];
  ...
  if (state.transition) {
    let travel = frameTravel(metrics);
    let step = Math.round(transitionProgress() * travel);
    let fromOffset = -step * state.transition.direction;
    let toOffset = (travel - step) * state.transition.direction;

    pushFloatingFrame(ops, "from", slideSpecs[state.transition.fromIndex], metrics, fromOffset);
    pushFloatingFrame(ops, "to", slideSpecs[state.transition.toIndex], metrics, toOffset);
  } else {
    pushFloatingFrame(ops, "current", slideSpecs[state.currentSlide], metrics, 0);
  }
...
function pushFloatingFrame(
  ...
  for (let index = 0; index < rows.length; index++) {
    let row = rows[index];
    ops.push(
      open(`frame-row-${key}-${index}`, { layout: { width: grow(), height: fixed(1) }, bg: palette.frameBg }),
...
function drawGridRoom(canvas: string[][]): void {
  ...
  write(canvas, cardX + 2, cardY + 1, "floating frame");
